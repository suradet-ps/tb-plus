...
use tb_logic::alerts::compute_alerts_for_patient;
use tb_models::patient::{ActivePatientRow, EnrollmentInput, PatientDetail};
...
pub async fn enroll_patient(
  ...
  tb_database::sqlite::enroll_patient(&sqlite, &enrollment)
    .await
...
pub async fn get_active_patients(
  ...
  let patients = tb_database::sqlite::get_active_patients(&sqlite)
    .await
    .map_err(|e| e.to_string())?;
  ...
  for patient in patients {
    let demographics = if let Some(pool) = mysql_pool {
      tb_database::mysql::get_patient_demographics(pool, &patient.hn)
        .await
        .ok()
        .flatten()
    } else {
    ...
    let current_plan = tb_database::sqlite::get_current_treatment_plan(&sqlite, &patient.hn)
      .await
      .ok()
      .flatten();

    let first_start = tb_database::sqlite::get_first_phase_start(&sqlite, &patient.hn)
      .await
      .ok()
      .flatten();
    ...
    let total_months = tb_database::sqlite::get_all_treatment_plans(&sqlite, &patient.hn)
      .await
      .ok()
      .map(|plans| plans.iter().map(|p| p.duration_months).sum::<i64>());

    let days_since_last = if let Some(pool) = mysql_pool {
      tb_database::mysql::get_last_dispensing_date(pool, &patient.hn, &all_icodes)
        .await
        .ok()
        .flatten()
...
pub async fn get_patient_detail(
  ...
  // ── Demographics (HOSxP) ─────────────────────────────────────────────────
  let demographics = if let Some(pool) = mysql_pool {
    match tb_database::mysql::get_patient_demographics(pool, &hn).await {
      Ok(demo) => demo,
      Err(e) => {
  ...
  // ── SQLite data ──────────────────────────────────────────────────────────
  let current_plan = tb_database::sqlite::get_current_treatment_plan(&sqlite, &hn)
    .await
    .ok()
    .flatten();

  let followups = tb_database::sqlite::get_followups(&sqlite, &hn)
    .await
    .map_err(|e| e.to_string())?;

  let outcome = tb_database::sqlite::get_outcome(&sqlite, &hn)
    .await
    .ok()
    .flatten();
  ...
  let dispensing_history = if let Some(pool) = mysql_pool {
    match tb_database::mysql::get_dispensing_history(pool, &hn, &all_icodes, &icode_to_class_map)
      .await
    {
      Ok(rows) => rows,
      Err(e) => {
  ...
  let days_since_last = if let Some(pool) = mysql_pool {
    match tb_database::mysql::get_last_dispensing_date(pool, &hn, &all_icodes).await {
  ...
  let first_start = tb_database::sqlite::get_first_phase_start(&sqlite, &hn)
    .await
    .ok()
    .flatten();
  ...
  let total_months = tb_database::sqlite::get_all_treatment_plans(&sqlite, &hn)
    .await
    .ok()
    .map(|plans| plans.iter().map(|p| p.duration_months).sum::<i64>());
...
pub async fn discharge_patient(
  sqlite: State<'_, SqlitePool>,
  outcome: tb_models::treatment::OutcomeInput,
) -> Result<(), String> {
  tb_database::sqlite::discharge_patient(&sqlite, &outcome)
    .await
    .map_err(|e| e.to_string())
}
...
pub async fn get_discharged_patients(
  ...
  let patients = tb_database::sqlite::get_discharged_patients(&sqlite)
    .await
    .map_err(|e| e.to_string())?;
  ...
  for patient in patients {
    ...
    let all_plans = tb_database::sqlite::get_all_treatment_plans(&sqlite, &patient.hn)
      .await
      .ok()
      .unwrap_or_default();
    ...
    let outcome_value = tb_database::sqlite::get_outcome(&sqlite, &patient.hn)
      .await
      .ok()
      .flatten()
      .map(|o| o.outcome);
