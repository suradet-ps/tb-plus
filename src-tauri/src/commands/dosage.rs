...
use crate::models::settings::{DosageRule, RegimenEntry, RegimenPhase};
...
pub async fn get_configured_dosage_drugs(
  ...
  db::mysql::get_drug_items_by_icodes(pool, &class_by_icode)
    .await
    .map_err(|e| e.to_string())
}
...
pub async fn assess_patient_dosage(
  ...
  let patient = db::mysql::get_patient_latest_weight_summary(pool, &hn)
    .await
    .map_err(|e| e.to_string())?
...
async fn load_candidate_map(
  ...
  let class_by_icode = settings
    ...
    .await
    .map_err(|e| e.to_string())?;
  let candidates = db::mysql::get_drug_items_by_icodes(pool, &class_by_icode)
    .await
    .map_err(|e| e.to_string())?;
  Ok(
    candidates
...
fn normalized_regimen_phases(regimen: &RegimenEntry) -> Vec<RegimenPhase> {
...
fn parse_regimen_durations(regimen_name: &str) -> (u32, u32) {
...
fn build_assessment_result(
  patient: crate::models::dosage::DosagePatientSummary,
...
fn parse_strength_mg(strength_text: &str) -> Option<f64> {
